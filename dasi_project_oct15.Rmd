---
title: "Financial Background: Shaping attitude towards Improving & Protecting the Environemnt"
date: "25/10/2015"
output:
  html_document:
    theme: cerulean
...
Both are Ordinal (Ordered Categorical) variables. "incom16" has three level "Too Little", "About Right" & "Too Much" while "natenvir" has six levels “Far Below Average”, “Below Average”, “Average”, “Above Average”, “Far Above Average” & "Lived in Institution". The last variable "Lived in Institution" does not have any respondent in the period of our study so this is being dropped & we are treating our data for income level at 16 to be of 5 levels. 
...
2) Responds who perceive themselves to be coming from financially stronger backgrounds feel more strongly that there i Too Little Investment. 59% in the far below average group growing to 67% in the Above Average & Far above average groups.   
...
For a future research it will be good to see if this decreasing trend of the percentage who belive that Too Little investment is being done is within the range of statistical probabilities or is this a real trend.  

5) For the purpose of our current interest we can see a correlation between the perceived family income level at 16 & the attitude toward investment in improving & protecting the environment. We will proceed with a Chi-Square test.    

### Inference:
We are trying to infer the correlation between two Categorical Variables. The firt one has 5 levels of self perceived income level of the family when the respondent was 16 years old. The second caputres three levels on the respondents attitude towards investment on Improving & Protecting the environment. We will need to do a CHI-SQUARE INDEPENDENCE TEST. Our input data is as given below:
...
#### Hypotheses    
Null Hypotheses H0: For a Citizen of the United States there is no correlation between their family income level at the age of 16 and their attitude towards the Investment in Improving & Protecting the climate.  
Alternate Hypothesis Ha: The attitude of an American citizen towards the investment being done for improving and protecting the environment is linked to their own perception of their family income level when they were at an age of 16 years.
...
1) Independence: 
    ...
    - Each case only contributes to one cell. So each case is one of 15 unique combinations of income level (5 levels) & attitude to environment (3 levels). Put another way eevry respondent is uniquely at a specific level for each of the two categorical variables so a person cannot be both Average & Above Average or say that Too Little is being done & just enough is being done, those responses are not possible so the condition is satisifed.    
...
The expected count is calculated as given below:  
  ...
  Expected count(row 2; col 2) = 1634*1807 / 6377 = 463.01  
To find Chi Square Test Statistic  first for each table count we calculate the (observed count - expected count)^2 / expected count e.g. for the 1st row, 1st column we have (352 - 375.495)^2/ 375.495 = 1.47. For all the Table counts we add up this value & arrive at our X^2 the Chi Square Statistic which in this case is 22.99841.  
  The degree of freedom df = (number of rows minus 1) * (number of columns minus 1) = 4*2 = 8  
